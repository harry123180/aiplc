...
function highlight(line) {
  ...
  while ((m = re.exec(code)) !== null) {
    ...
    if (m[1]) cls = 'str';else
    if (m[2]) cls = 'num';else
    if (m[3]) {cls = HAL.has(m[3]) ? 'fn' : KW.has(m[3]) ? 'kw' : 'id';} else
    if (m[4]) cls = 'ws';
...
function ChatMock({ onClose, onExpand, expanded, fullscreen, density }) {
      ...
      <div className="panel-head" style={{ background: 'var(--qp-bg)', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', lineHeight: 1.15 }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--qp-text)' }} data-comment-anchor="8d95f14458-span-189-13">千鉑助手</span>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          ...
          {onClose &&
          <button className="qp-icon-btn" style={{ width: 28, height: 28 }} onClick={onClose} title="收合 AI Chat">
              <Icon name="x" size={15} />
            </button>
          }
        </div>
