...
/* ───────── Chat dock (right / bottom / float / fullscreen + collapsed rail) ───────── */
function ChatRail({ onExpand }) {
  return (
    <div className="chat-rail" onClick={onExpand} title="展開 AI Chat">
      <div className="chat-rail-icon"><Ic name="sparkles" size={17} color="#fff" /></div>
      <div className="chat-rail-label">AI 助手</div>
      <Ic name="chevLeft" size={15} color="var(--qp-text-dim)" />
    </div>
  );
}

function ChatDock({ pos, open, setOpen, fs, setFs, width, setWidth, height, setHeight, collapsed, setCollapsed, density }) {
  ...
  const panel = (extraStyle, opts = {}) => (
    <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'var(--qp-bg)', ...extraStyle }}>
      <ChatP onClose={() => { setFs(false); setOpen(false); }}
             onExpand={opts.canFs ? () => setFs(!fs) : null}
             onCollapse={opts.canCollapse ? () => setCollapsed(true) : null}
             fullscreen={fs} density={density} />
    </div>
  );
  ...
  if (pos === 'right') {
    if (collapsed) return <ChatRail onExpand={() => setCollapsed(false)} />;
    return (
      <>
        <div className="resize-x" onMouseDown={onWDrag} />
        {panel({ width, flexShrink: 0, borderLeft: '1px solid var(--qp-border)' }, { canFs: true, canCollapse: true })}
      </>
    );
  }
  if (pos === 'bottom') {
  ...
  return (
    <div style={{ position: 'relative', height: '100%', display: 'flex', flexDirection: 'column', background: 'var(--qp-bg-alt)' }}>
      ...
      <div style={{ flex: 1, minHeight: 0, display: 'flex', position: 'relative' }}>
        <div style={{ flex: 1, minWidth: 0, padding: 14, paddingTop: 64 }}>
        ...
        {chatOpen && chatPos === 'right' && (
          ctx.chat.collapsed ? (
            <div style={{ padding: '64px 14px 14px 0' }}>
              <ChatRail onExpand={() => ctx.chat.setCollapsed(false)} />
            </div>
          ) : (
            <div style={{ width: ctx.chat.width, flexShrink: 0, padding: '64px 14px 14px 0' }}>
              <div style={{ height: '100%', borderRadius: 'var(--qp-r-2xl)', overflow: 'hidden', border: '1px solid var(--qp-border)', boxShadow: 'var(--qp-shadow)' }}>
                <ChatP onClose={() => setChatOpen(false)} onExpand={() => ctx.chat.setFs(!ctx.chat.fs)} onCollapse={() => ctx.chat.setCollapsed(true)} fullscreen={ctx.chat.fs} />
              </div>
            </div>
          )
        )}
...
function App() {
  ...
  const [chatCollapsed, setChatCollapsed] = u(false);
  ...
  const setSim = (v) => {
    setSimRaw(v);
    if (v) { setView('circuit'); setBottomOpen(true); setBottomTab('serial'); }
    else { setBottomTab('console'); }
  };

  const ctx = {
    view, setView, sim, setSim, chatOpen, setChatOpen, chatPos: t.chatPos,
    chat: { open: chatOpen, setOpen: setChatOpen, fs, setFs, width: chatW, setWidth: setChatW, height: chatH, setHeight: setChatH, collapsed: chatCollapsed, setCollapsed: setChatCollapsed, density: t.density },
